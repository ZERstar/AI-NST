...
def convert_to_base64(img):
    # array = arr.numpy()
    byte_string = base64.b64encode(img)
    return byte_string
...
def style_transfer_image(
    ...
    for n in range(epochs):
        ...
        for m in range(steps_per_epoch):
            ...
            train_step(image)
        img_bytes = tensor_to_image(image)
        # Store the image data in the Celery backend
...
#     k = (style_transfer_image(
#         "painting.JPG", "Ub.JPG", save_name="blend-weighted",
#         style_weight=1e-2, content_weight=3e4, total_variation_weight=30,
#     ))

#     img_bytes = io.BytesIO(k)
